	...
	<table class="widefat" align="center">
		...
		<?php foreach($dependencies as $dependency):?>
			<tr><td><?php _e('The exam', 'watupro')?> <strong><?php echo $dependency->exam?></strong> must be completed 
				<strong><?php if(strstr($dependency->final_screen, "%%POINTS%%")): 
					printf(__('with at least %d', 'watupro'), $dependency->depend_points);
					if(!empty($dependency->mode) and $dependency->mode == 'percent'): _e('% correct answers', 'watupro'); 
