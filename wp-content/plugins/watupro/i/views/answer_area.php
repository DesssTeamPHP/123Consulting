<div class="postbox" id="sortAnswerArea" style="display:<?php echo (!empty($question) and $question->answer_type=='sort')?'block':'none';?>">
	...
	<div class="inside">
		<p><?php _e('For sorting questions simply enter your values one per line in the correct way they should be sorted.<br> Feel free to leave blank lines for your own clarity.<br> If a value needs to contain a new line, use the HTML &lt;br&gt; tag and not a real new line.', 'watupro')?></p>
...
<div class="postbox" id="matrixAnswerArea" style="display:<?php echo (!empty($question) and $question->answer_type=='matrix')?'block':'none';?>">
	<h3 class="hndle">&nbsp;<span><?php _e('Answers', 'watupro') ?></span></h3>
	<div class="inside">
		<p><?php _e('Place the match criteria at left. These will be shown to the user in the order you enter them. Place the correct matches at right. These will be mixed so the user can drag and drop them.', 'watupro')?></p>
		<?php if(!empty($matches) and sizeof($matches)):
		foreach($matches as $match):
			// existing matches ?>
			<p class="wtp-notruefalse">
				<textarea rows="4" cols="30" name="left_match_<?php echo $match['id']?>"><?php echo stripslashes($match['left']); ?></textarea> 
				=
				<textarea rows="4" cols="30" name="right_match_<?php echo $match['id']?>"><?php echo stripslashes($match['right']); ?></textarea>
			</p>
		<?php endforeach;
		endif; ?>
		<p "class='wtp-notruefalse'">
