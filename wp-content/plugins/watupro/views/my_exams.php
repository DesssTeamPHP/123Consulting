<h1><?php printf(__("My %s", 'watupro'), __('Courses', 'watupro'));?></h1>

<?php if($user_id != $user_ID):?>
	<p><?php printf(__('Showing %s of ', 'watupro'), __('quizzes', 'watupro'))?> <strong><?php echo $user->user_login?></strong></p>
<?php endif;?>

<h2><?php printf(__("%s courses", 'watupro'), __('Available', 'watupro'))?></h2>
<?php if($num_to_take):?>
	<table class="widefat">
	<tr><th><?php printf(__('%s title', 'watupro'), __('Courses', 'watupro'))?></th><th><?php _e("Category", 'watupro')?></th></tr>
...
<h2><?php _e('Completed courses', 'watupro')?></h2>
<?php if($num_taken):?>
	<table class="widefat">
	<tr><th><?php _e('Courses title', 'watupro')?></th><th><?php _e('Points', 'watupro')?></th>
