...
<div id="poststuff">
	...
	<div class="postbox watupro-tab-div" id="settings" style="display:none;">
	    ...
	    <p>
		<?php _e('Set time limit of', 'watupro')?> 
        <?php
		$m=@$dquiz->time_limit;
		if($m!=0) 
		{
		...
		?>
        <input type="text" name="time_limit" size="17" value="<?php echo $expm;?>"> 
		<?php _e('minutes (Leave it blank or enter 0 to not set any time limit.)', 'watupro')?>
        </p>
		...
		<p><input type="checkbox" name="dont_display_question_numbers" value="1" <?php if(!empty($advanced_settings['dont_display_question_numbers'])) echo 'checked'?>> <?php _e('Do not display question numbers.', 'watupro')?></p>		
			...
			<label><?php _e('Schedule from:', 'watupro')?></label> &emsp;
	                <input type="text" name="schedule_from" class="watuproDatePicker" value="<?php echo $schedule_from?>" onchange="return changedate();">
	                &nbsp;
